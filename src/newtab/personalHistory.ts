...
export function initializePersonalHistory(refresh: () => void) {
  ...
  function controls() { file.disabled = busy || !ready; save.disabled = busy || !ready || pending === undefined; exportButton.disabled = busy || !ready; }
  ...
  file.onchange = async () => {
    const token = ++revision; pending = undefined; preview.replaceChildren(); controls();
    const selected = file.files?.[0]; if (!selected) { status.textContent = '請選擇 JSON 檔案。'; return; }
    try {
      if (selected.size > 5 * 1024 * 1024) throw new Error('檔案不得超過 5 MB。');
      const content = await selected.text(); if (token !== revision) return;
      const entries = parsePersonalHistory(JSON.parse(content.replace(/^\uFEFF/, '')));
      pending = entries;
      ...
      status.textContent = `已驗證 ${entries.length} 筆。按「匯入並取代」會取代目前的個人紀事${entries.length ? '，台灣及國際資料不受影響。' : '並清空清單。'}`;
    } catch (error) { status.textContent = error instanceof SyntaxError ? 'JSON 格式錯誤，請確認檔案內容。' : error instanceof Error ? error.message : '讀取失敗。'; }
    controls();
  };
  save.onclick = async () => {

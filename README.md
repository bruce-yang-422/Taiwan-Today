...
- 五種日曆樣式：經典極簡、現代、傳統、月曆工作台、閱讀日曆，皆可搭配亮色／暗色。
- 經典極簡保留最初的米色背景、置中日期與時鐘、小型捷徑及無卡片外框的歷史紀事。
- 月曆工作台：左側網站捷徑、中央國曆與農曆月曆、右側日期、大指針時鐘及待辦；可切換月份、選取日期查看農曆與歷史，或按「今天」返回。時鐘、待辦與每日一句仍以今天為準，待辦共用同一份清單。
- 閱讀日曆：深藍日期側欄、每日一句、歷史時間軸，底部提供搜尋與捷徑。每日一句可在外觀設定選擇「每日提醒、聖嚴法師語錄、負能量（幽默）、四書五經」，於月曆工作台與閱讀日曆顯示；分類自動保存，依台灣日期在所選分類內輪替。
- 現代樣式採灰藍背景與圓角卡片；寬螢幕並排日期、時鐘、待辦，下方提供膠囊搜尋列、捷徑卡片與歷史卡片，支援暗色模式。
...
- 預設捷徑包含 YouTube、PChome、GitHub、ChatGPT；更新時會補上 ChatGPT，保留既有捷徑，手動刪除後不會再次加入。
- 外觀使用圖示搭配滑動分段控制（系統／亮色／暗色），支援鍵盤方向鍵與減少動態效果偏好；另可選現代／傳統樣式，設定持久保存。
- 傳統樣式採紙曆排版、朱紅年份橫幅、雙線框及明體字；寬螢幕以日期／大時鐘／待辦並排，支援亮色紙面與深色配色。
